...
void setup() {
  ...
  //Serial2.println("Restarted Program");
}
...
void PickPP(float X1, float Y1, float Z1, float X2, float Y2, float Z2) {
  ...
  DK.inverse(X1, Y1, Z1 + 50);  // position
  ...
  DK.inverse(X2, Y2, Z2 + 30);  // position
  RunTheta(DK.a, DK.b, DK.c);
...
void Command(String Com) {
  ...
  if (Com == "DOWN") {
    ...
    //Serial.println(String(dTheta1) + "," + String(dTheta2) + "," + String(dTheta3));
    RunTheta(DK.a, DK.b, DK.c);
  }
  if (Com == "UP") {
    SetMaxSpeed(30, 50);
...
void SendMes() {
  if (send == 1) {
    ...
    Serial.println(String(DK.x) + " " + String(DK.y) + " " + String(DK.z)
                   + " " + String(dTheta1) + " " + String(dTheta2) + " " + String(dTheta3));
  }
}
...
void Home() {
  ...
  if (SOneLimitState == HIGH && RobotHomed == false
      && SOneLimitReached == false && STwoLimitReached == false && SThreeLimitReached == false) {
  ...
  if (STwoLimitState == HIGH && RobotHomed == false
      && SOneLimitReached == true && STwoLimitReached == false && SThreeLimitReached == false) {
    //move 1 degree at a time
    ////////////
    SetMaxSpeed(70, 50);
    RunTheta(0, -n2, 0);
  ...
  if (SThreeLimitState == HIGH && RobotHomed == false
      && SOneLimitReached == true && STwoLimitReached == true && SThreeLimitReached == false) {
    //move 1 degree at a time
    //////
    SetMaxSpeed(70, 50);
    RunTheta(0, 0, -n3);
